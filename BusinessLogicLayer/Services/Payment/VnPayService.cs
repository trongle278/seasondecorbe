...
namespace KCP.Service.Service.Pay
{
    public class VNPayService
    {
        ...
        public static string VNPay(HttpContext context, VnPayRequest vnPayRequest, bool isMobile = false)
        {
            // Determine the appropriate return URL based on platform
            ...
            vnpay.AddRequestData("vnp_Command", "pay");
            vnpay.AddRequestData("vnp_TmnCode", vnp_TmnCode);
            vnpay.AddRequestData("vnp_Amount", (vnPayRequest.Amount * 100).ToString()); // Số tiền thanh toán không mang các ký tự phân tách
            vnpay.AddRequestData("vnp_BankCode", "VNBANK");
            vnpay.AddRequestData("vnp_CreateDate", CreatedDate.ToString("yyyyMMddHHmmss"));
            vnpay.AddRequestData("vnp_CurrCode", "VND");
            vnpay.AddRequestData("vnp_IpAddr", Utils.GetIpAddress(context));
            ...
            // Set order info based on transaction type
            string orderInfo = $"Thanh toan {vnPayRequest.TransactionType} - {OrderId}";
